...
- [x] Every source attempt is recorded (`{kind, ref, ok}`), including the one for a PR that references no plan/spec at all (`ref: null`); a NAMED reference that could not be resolved becomes a `missing_context[]` entry and is never invented, while the unlinked-spec attempt is excluded from it — an absence is not a failure, and folding the two together would warn on nearly every PR — `server/src/modules/reviews/intent-helpers.ts:15` (`deriveMissingContext`), `:20` (`isUnlinkedSpec`), `server/src/modules/reviews/compose.ts:99` (the recorded absence) · test: `server/test/reviews-intent-helpers.test.ts` ("reports only failed attempts, with a readable message per kind", "reports an unreadable spec but not a PR that simply links none")
