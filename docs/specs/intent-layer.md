...
- [x] Sources are gathered deterministically — PR title, body, linked issue, a linked plan/spec resolved to a repo path and read via `container.git.readFile`, the changed-file list (hunk headers only, never diff bodies), and commit messages; each attempt is recorded and unreachable sources land in `missing_context[]`, never invented — `server/src/modules/reviews/compose.ts:46` (`RepoIntentSourceCollector.collect`) · test: `server/test/reviews-intent-sources.test.ts:114` ("reports an unreferenced/unresolved issue and spec as not available, never invented")
...
- [x] Intent is persisted keyed by `head_sha`; a run reuses the stored record when the head is unchanged and re-derives when it's missing or stale — `server/src/modules/reviews/run-executor.ts:377` (`deriveIntent` — stored/stale check) · untested
