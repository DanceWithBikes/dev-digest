...
export default async function pullsRoutes(appBase: FastifyInstance) {
  ...
  app.get('/repos/:id/pulls', { schema: { params: IdParams } }, async (req): Promise<PrMeta[]> => {
    ...
    // Latest-review SCORE and FINDINGS rollup per PR for the list's score ring
    // and findings column. Computed on read from reviews (no FK denorm); the
    ...
    // Findings of those latest reviews, grouped per review. Dismissed findings
    // are INCLUDED so the list's counts match the per-run severity pills on the
    // PR page (which render dismissed cards too, dimmed).
    const findingsByReview = new Map<string, (typeof t.findings.$inferSelect)[]>();
