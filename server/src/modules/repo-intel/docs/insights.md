...
- **2026-09-19 · "The facade never throws" only covers missing data** — `CLAUDE.md` says the facade never throws, but only `tryGetIndexState` catches; the other reads query the DB unguarded, so a DB error propagates. The reviews run-executor wraps each call in try/catch to compensate. Consumers in later lessons must do the same.
